...
class GraphMaker:
    ...
    def getSuperpixel(self):
        ...
        endtime = datetime.datetime.now()
        print("Cet superpixel: " + str((endtime - starttime).seconds))
        # init super-pixel
    ...
    def LABMap(self):
        ...
        # LAB_map_saved = LAB_map.copy()
        # LAB_map_saved[:, :, 0] = LAB_map_saved[:, :, 0] / 255 * 100
        # LAB_map_saved[:, :, 1] = LAB_map_saved[:, :, 1] - 128
        # LAB_map_saved[:, :, 2] = LAB_map_saved[:, :, 2] - 128
        cv2.imwrite('./results/LabImg.jpg', LAB_map.astype(uint8))
